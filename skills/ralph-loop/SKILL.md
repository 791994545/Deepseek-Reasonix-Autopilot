...
allowed-tools: run_command, read_file, search_files, write_file, edit_file
---

> ⚠️ **此 skill 已标记为 OpenCode 遗产**。Reasonix 中请用 `full-autonomous` v2.1 + `task-executor` 替代持久循环执行。
...
edit_file `.claude/hooks/stop-hook.sh`:
...
Default location is `plans/`. edit_file the start command to change:

```markdown
# In .claude/commands/ralph-loop.md
1. **read_file plans/progress.md** → Change to your path
