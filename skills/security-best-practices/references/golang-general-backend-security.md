...
Detection hints:
- search_content for `SELECT`, `INSERT`, `UPDATE`, `DELETE` and check how query strings are built.
...
Fix:
- Switch to `crypto/rand` (`rand.Reader`, `rand.read_file`, or secure token helpers).
