...
> **Reasonix 注意**: 此 skill 提供浏览器端到端体验测试（需 agent-browser），与内置 `review` 的代码审查不同。
...
read_file [references/issue-taxonomy.md](references/issue-taxonomy.md) for the full list of what to look for and the exploration checklist.
...
5. write_file numbered repro steps in the report, each referencing its screenshot.
...
write_file a brief description and reference the screenshot in the report. Set **Repro Video** to `N/A`.

---

**For all issues:**

1. **Append to the report immediately.** Do not batch issues for later. write_file each one as you find it so nothing is lost if the session is interrupted.
...
- **write_file repro steps that map to screenshots.** Each numbered step in the report should reference its corresponding screenshot. A reader should be able to follow the steps visually without touching a browser.
...
- **write_file findings incrementally.** Append each issue to the report as you discover it. If the session is interrupted, findings are preserved. Never batch all issues for the end.
