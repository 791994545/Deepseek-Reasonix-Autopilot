...
allowed-tools: run_command, read_file, write_file, edit_file, search_files, search_content, TaskCreate, TaskUpdate, TaskList
...
9. Run the command using run_command **with `run_in_background: true`** - this prevents the timeout indicator in Claude Code UI
10. The external script will handle everything in the background

**Important:** Fresh-context mode launches an external bash script that spawns new Claude sessions. Use `run_in_background: true` when calling the run_command tool so the script runs in the background without blocking. Monitor progress with `./scripts/ralph/ralph-status.sh --watch` or the auto-opened monitor window. Do NOT create a state file for fresh mode.
...
**Step 4.5: read_file Guardrails**

read_file `plans/guardrails.md` if it exists. This file contains learned constraints ("signs") from previous iterations that prevent repeated failures. Include guardrails content in the state file so they're visible during work.
...
1. read_file the context files (progress.md, guardrails.md, prd.json, plan_file if specified)
