...
- **sessions_history** — read_file another session's transcript  
...
1. read_file `simplify_and_harden.learning_loop.candidates` from the task summary.
...
write_file promoted rules as short prevention rules (what to do before/while coding),
...
| `scripts/error-detector.sh` | PostToolUse (run_command) | Triggers on command errors |
...
5. **Verify**: read_file skill in fresh session to ensure it's self-contained
