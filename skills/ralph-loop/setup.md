...
**Option B:** edit_file `.claude/hooks/stop-hook.sh` and change `VERIFY_COMMAND`:
...
edit_file the paths in `stop-hook.sh`:
...
edit_file `.claude/commands/ralph-loop.md` to match your context file paths:

```markdown
1. **read_file plans/progress.txt** → Change to your path
